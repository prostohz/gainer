...
export const BacktestTradesByDateTimeHistogram = ({ trades }: { trades: TCompleteTrade[] }) => {
  ...
  const binsCount = (24 * 60) / intervalMinutes;

  const intervalBins = new Map<number, TCompleteTrade[]>();

  // Инициализируем все интервалы (96 интервалов по 15 минут в сутках)
  for (let interval = 0; interval < binsCount; interval++) {
    intervalBins.set(interval, []);
  }

  // Распределяем сделки по 15-минутным интервалам
  trades.forEach((trade) => {
    const openTime = dayjs(trade.openTime);
    const totalMinutes = openTime.hour() * 60 + openTime.minute();
          ...
          {displayMode === 'trades'
            ...
            : displayMode === 'averageRoi'
              ...
              : displayMode === 'totalRoi'
                ? 'Total ROI'
                : 'Win Rate'}{' '}
          Of The Day By {intervalMinutes}-Minute Intervals
        </h3>
